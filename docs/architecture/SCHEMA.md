...
| Late fees        | late_fee_enabled           | boolean          | no   | Default `false`. Deliberately off: an upgrade must not start charging a self-hoster's clients.                                                                                           |
|                  | late_fee_type              | enum             | yes  | `percentage \| fixed`. Null until a fee is configured; the type names which amount column below carries it.                                                                              |
|                  | late_fee_percentage        | numeric(5, 2)    | yes  | 0–100. Taken on the outstanding balance at the moment of assessment, not on the invoice total.                                                                                           |
...
|                  | stripe_publishable_key     | text             | yes  | Stored, and read by no payment path: checkout is hosted by Stripe and runs server-side                                                                                                   |
|                  | stripe_secret_key          | text (encrypted) | yes  | Opens the Checkout Session; with `stripe_webhook_secret`, gates the public card affordance                                                                                               |
|                  | stripe_webhook_secret      | text (encrypted) | yes  | Verifies the event that records a card payment; without it no card payment is offered                                                                                                    |
...
- `chk_settings_late_fee_shape` — the same either-or shape as `chk_invoices_discount_shape`: either
  the type and both amounts are null, or the type is `percentage` with only a percentage, or it is
  `fixed` with only an amount.
- `chk_settings_late_fee_enabled_shape` — `late_fee_enabled = false OR late_fee_type IS NOT NULL`.
- `chk_settings_late_fee_percentage` — null or `>= 0 AND <= 100`.
...
| late_fee_cents              | bigint          | yes  |                     | The late fee charged on this invoice, and already included in `total_cents`. Null until the nightly overdue sweep charges one; `0` means charged and then waived, which is why the sweep never reassesses a non-null value. Written by `features/invoices/lateFees.ts` and by the owner's adjustment in `features/invoices/mutations.ts` |
| notes                       | text            | yes  |                     |                                                                                                                                                                                                                                                                                                                                          |
