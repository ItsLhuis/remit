...
| portal_token              | text             | yes  |                     | Unique where not null. The client portal's bearer credential, minted on demand by `rotateClientPortalLink`. Null is both "no portal yet" and "portal revoked" — one state (ADR-0029) — and `softDeleteClient` clears it |
