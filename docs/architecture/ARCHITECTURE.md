...
The history before v1 was squashed into one baseline: `0000_initial_schema.sql` is the whole schema
...
[ADR-0026](adr/0026-document-parentage.md)). Squashing is a pre-v1 one-off: from the first release
onwards, migrations are only ever appended, because a shipped instance replays them by hash.
