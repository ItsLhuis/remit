...
export type ProposalLineTotals = {
...
export function calculateProposalTotal(
  ...
  proposalDiscount?: ProposalDiscount | null
): ProposalTotals {
  const allocation = allocate(lineItems, proposalDiscount ?? null)
...
// lines sum exactly to the proposal's `totalCents` even when a document discount is present.
export function calculateProposalLineTotals(
  lineItems: ProposalLineItemInput[],
  proposalDiscount?: ProposalDiscount | null
): ProposalLineTotals[] {
  return allocate(lineItems, proposalDiscount ?? null).lines.map((line) => ({
...
function applyDiscount(baseCents: number, discount: ProposalDiscount | null): number {
