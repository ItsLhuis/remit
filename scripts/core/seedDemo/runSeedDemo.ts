...
export function getSeedDemoHelpText(): string {
...
export function formatSeedSizeLine(plan: DemoSeedPlan): string {
  if (plan.size !== "custom") {
...
export function formatPlannedSummary(plan: DemoSeedPlan): string {
  return [
...
export function formatSamplePreview(plan: DemoSeedPlan): string {
...
export function formatExistingRowsInline(counts: ReseedTableCounts): string {
...
export function formatExistingRowsList(counts: ReseedTableCounts): string {
  return Object.entries(counts)
    .filter(([, value]) => value > 0)
    .map(([table, value]) => `  ${table}: ${value}`)
    .join("\n")
